...
public extension ReposNamespace {
    /// Lists the commit comments for a specified repository. Comments are ordered by ascending ID. This endpoint
    ...
    func listCommitCommentsForRepo(
        owner: String,
        repo: String,
        perPage: Int?,
        page: Int?
    ) async throws -> [CommitComment] {
        try await ReposMethods.reposListCommitCommentsForRepo(
            config: config,
            owner: owner,
            repo: repo,
            perPage: perPage,
    ...
    /// Gets a specified commit comment. This endpoint supports the following custom media types. For more information,
    /// see "[Media
    /// types](https://docs.github.com/rest/using-the-rest-api/getting-started-with-the-rest-api#media-types)." -
    /// **`application/vnd.github-commitcomment.raw+json`**: Returns the raw markdown body. Response will include
    /// `body`. This is the default if you do not pass any specific media type. -
    ...
    /// **`application/vnd.github-commitcomment.full+json`**: Returns raw, text, and HTML representations. Response will
    /// include `body`, `body_text`, and `body_html`.
    func getCommitComment(owner: String, repo: String, commentId: Int) async throws -> CommitComment {
        try await ReposMethods.reposGetCommitComment(config: config, owner: owner, repo: repo, commentId: commentId)
    }

    /// Updates the contents of a specified commit comment. This endpoint supports the following custom media types. For
    ...
    /// **`application/vnd.github-commitcomment.full+json`**: Returns raw, text, and HTML representations. Response will
    /// include `body`, `body_text`, and `body_html`.
    func updateCommitComment(owner: String, repo: String, commentId: Int, body: String) async throws -> CommitComment {
        try await ReposMethods.reposUpdateCommitComment(
            config: config,
            owner: owner,
            repo: repo,
            commentId: commentId,
    ...
    /// body.
    func deleteCommitComment(owner: String, repo: String, commentId: Int) async throws -> SdkEmptyResponse {
        try await ReposMethods.reposDeleteCommitComment(config: config, owner: owner, repo: repo, commentId: commentId)
    }
    ...
    func listCommits(options: ReposMethods.ReposListCommitsOptions) async throws -> [Commit] {
        try await ReposMethods.reposListCommits(config: config, options: options)
    }
    ...
    /// documentation. Returns all branches where the given commit SHA is the HEAD, or latest commit for the branch.
    func listBranchesForHeadCommit(owner: String, repo: String, commitSha: String) async throws -> [BranchShort] {
        try await ReposMethods.reposListBranchesForHeadCommit(
            config: config,
            owner: owner,
            repo: repo,
            commitSha: commitSha
    ...
    /// Lists the comments for a specified commit. This endpoint supports the following custom media types. For more
    ...
    func listCommentsForCommit(
        ...
        try await ReposMethods.reposListCommentsForCommit(
            config: config,
            owner: owner,
            repo: repo,
            commitSha: commitSha,
    ...
    /// Create a comment for a commit using its `:commit_sha`. Access to commit comments can be controlled by
    /// organization owners. For more information, see "[Managing commit comments for your organization](https://docs.github.com/organizations/managing-organization-settings/managing-commit-comments-for-your-organization)".
    ...
    func createCommitComment(
        ...
        try await ReposMethods.reposCreateCommitComment(
            config: config,
            owner: owner,
            repo: repo,
            commitSha: commitSha,
...
public extension ReposNamespace {
    ...
    func listPullRequestsAssociatedWithCommit(
        owner: String,
        repo: String,
        commitSha: String,
        perPage: Int?,
        ...
        try await ReposMethods.reposListPullRequestsAssociatedWithCommit(
            config: config,
            owner: owner,
            repo: repo,
            commitSha: commitSha,
    ...
    func getCommit(owner: String, repo: String, ref: String, page: Int?, perPage: Int?) async throws -> Commit {
        try await ReposMethods.reposGetCommit(
            config: config,
            owner: owner,
            repo: repo,
            ref: ref,
    ...
    func getCombinedStatusForRef(
        ...
        try await ReposMethods.reposGetCombinedStatusForRef(
            config: config,
            owner: owner,
            repo: repo,
            ref: ref,
    ...
    func listCommitStatusesForRef(
        ...
        try await ReposMethods.reposListCommitStatusesForRef(
            config: config,
            owner: owner,
            repo: repo,
            ref: ref,
    ...
    /// Returns all community profile metrics for a repository. The repository cannot be a fork. The returned metrics
    /// include an overall health score, the repository description, the presence of documentation, the detected code of
    /// conduct, the detected license, and the presence of ISSUE\_TEMPLATE, PULL\_REQUEST\_TEMPLATE, README, and
    /// CONTRIBUTING files. The `health_percentage` score is defined as a percentage of how many of the recommended
    /// community health files are present. For more information, see "[About community profiles for public repositories](https://docs.github.com/communities/setting-up-your-project-for-healthy-contributions/about-community-profiles-for-public-repositories)."
    /// `content_reports_enabled` is only returned for organization-owned repositories.
    func getCommunityProfileMetrics(owner: String, repo: String) async throws -> CommunityProfile {
        try await ReposMethods.reposGetCommunityProfileMetrics(config: config, owner: owner, repo: repo)
    }

    /// Compares two commits against one another. You can compare refs (branches or tags) and commit SHAs in the same
    /// repository, or you can compare refs and commit SHAs that exist in different repositories within the same
    /// repository network, including fork branches. For more information about how to view a repository's network, see
    ...
    /// This endpoint is equivalent to running the `git log BASE..HEAD` command, but it returns commits in a different
    /// order. The `git log BASE..HEAD` command returns commits in reverse chronological order, whereas the API returns
    /// commits in chronological order. This endpoint supports the following custom media types. For more information,
    /// see "[Media
    /// types](https://docs.github.com/rest/using-the-rest-api/getting-started-with-the-rest-api#media-types)." -
    /// **`application/vnd.github.diff`**: Returns the diff of the commit. - **`application/vnd.github.patch`**: Returns
    /// the patch of the commit. Diffs with binary data will have no `patch` property. The API response includes details
    /// about the files that were changed between the two commits. This includes the status of the change (if a file was
    /// added, removed, modified, or renamed), and details of the change itself. For example, files with a `renamed`
    /// status have a `previous_filename` field showing the previous filename of the file, and files with a `modified`
    /// status have a `patch` field showing the changes made to the file. When calling this endpoint without any paging
    /// parameter (`per_page` or `page`), the returned list is limited to 250 commits, and the last commit in the list
    /// is the most recent of the entire comparison. **Working with large comparisons** To process a response with a
    /// large number of commits, use a query parameter (`per_page` or `page`) to paginate the results. When using
    /// pagination: - The list of changed files…
    func compareCommits(
        owner: String,
        repo: String,
        basehead: String,
        ...
        try await ReposMethods.reposCompareCommits(
            config: config,
            owner: owner,
            repo: repo,
            basehead: basehead,
    ...
    /// Retrieves a file, directory, symlink, or submodule from a repository. Supply the repository identifiers and the
    /// target `path`, with `ref` optionally selecting a commit, branch, or tag instead of the default branch. Directory
    /// responses are limited to 1,000 files, and files larger than 100 MB are not supported.
    ///
    /// Gets the contents of a file or directory in a repository. Specify the file path or directory with the `path`
    /// parameter. If you omit the `path` parameter, you will receive the contents of the repository's root directory.
    /// This endpoint supports the following custom media types. For more information, see "[Media
    /// types](https://docs.github.com/rest/using-the-rest-api/getting-started-with-the-rest-api#media-types)." -
    /// **`application/vnd.github.raw+json`**: Returns the raw file contents for files and symlinks. -
    /// **`application/vnd.github.html+json`**: Returns the file contents in HTML. Markup languages are rendered to HTML
    /// using GitHub's open-source [Markup library](https://github.com/github/markup). -
    /// **`application/vnd.github.object+json`**: Returns the contents in a consistent object format regardless of the
    /// content type. For example, instead of an array of objects for a directory, the response will be an object with
    /// an `entries` attribute containing the array of objects. If the content is a directory, the response will be an
    /// array of objects, one object for each item in the directory. When listing the contents of a directory,
    /// submodules have their "type" specified as "file". Logically, the value _should_ be "submodule". This behavior
    /// exists [for backwards compatibility purposes](https://git.io/v1YCW). In the next major version of the API, the
    /// type will be returned as "submodule". If the content is a symlink and the symlink's target is a normal file in
    /// the repository, then the API responds with the content of the file. Otherwise, the API responds with an object
    /// describing the symlink itself. If the content is a submodule, the `submodule_git_url` field identifies the
    /// location of the submodule repository, and the `sha` identifies a specific commit within the submodule
    /// repository. Git uses the given URL when cloning the submodule repository, and checks out the submodule at that
    /// specific commit. If the submodule repository is not hosted on github.com, the Git URLs…
    func getContent(owner: String, repo: String, path: String, ref: String?) async throws -> ContentTree {
        try await ReposMethods.reposGetContent(config: config, owner: owner, repo: repo, path: path, ref: ref)
    }
    ...
    func createOrUpdateFileContents(options: ReposMethods
        .ReposCreateOrUpdateFileContentsOptions) async throws -> FileCommit {
        try await ReposMethods.reposCreateOrUpdateFileContents(config: config, options: options)
    }
}
...
public extension ReposNamespace {
    ...
    /// serially instead.
    func deleteFile(options: ReposMethods.ReposDeleteFileOptions) async throws -> FileCommit {
        try await ReposMethods.reposDeleteFile(config: config, options: options)
    }
    ...
    func listContributors(
        ...
        try await ReposMethods.reposListContributors(
            config: config,
            owner: owner,
            repo: repo,
            anon: anon,
    ...
    func listDeployments(options: ReposMethods.ReposListDeploymentsOptions) async throws -> [Deployment] {
        try await ReposMethods.reposListDeployments(config: config, options: options)
    }
    ...
    func createDeployment(options: ReposMethods.ReposCreateDeploymentOptions) async throws -> Deployment {
        try await ReposMethods.reposCreateDeployment(config: config, options: options)
    }
    ...
    func getDeployment(owner: String, repo: String, deploymentId: Int) async throws -> Deployment {
        try await ReposMethods.reposGetDeployment(config: config, owner: owner, repo: repo, deploymentId: deploymentId)
    }
    ...
    func deleteDeployment(owner: String, repo: String, deploymentId: Int) async throws -> SdkEmptyResponse {
        try await ReposMethods.reposDeleteDeployment(
            config: config,
            owner: owner,
            repo: repo,
    ...
    func listDeploymentStatuses(
        ...
        try await ReposMethods.reposListDeploymentStatuses(
            config: config,
            owner: owner,
            repo: repo,
            deploymentId: deploymentId,
    ...
    func createDeploymentStatus(options: ReposMethods
        .ReposCreateDeploymentStatusOptions) async throws -> DeploymentStatus {
        try await ReposMethods.reposCreateDeploymentStatus(config: config, options: options)
    }
}
...
public extension ReposNamespace {
    ...
    func getDeploymentStatus(
        ...
        try await ReposMethods.reposGetDeploymentStatus(
            config: config,
            owner: owner,
            repo: repo,
            deploymentId: deploymentId,
    ...
    /// this endpoint.
    func createDispatchEvent(
        owner: String,
        repo: String,
        eventType: String,
        clientPayload: [String: JSONValue]?
    ) async throws -> SdkEmptyResponse {
        try await ReposMethods.reposCreateDispatchEvent(
            config: config,
            owner: owner,
            repo: repo,
            eventType: eventType,
    ...
    /// Lists the environments for a repository. Anyone with read access to the repository can use this endpoint. OAuth
    /// app tokens and personal access tokens (classic) need the `repo` scope to use this endpoint with a private
    /// repository.
    func getAllEnvironments(
        ...
        perPage: Int?,
        page: Int?
    ) async throws -> ReposGetAllEnvironmentsResponse {
        try await ReposMethods.reposGetAllEnvironments(
            config: config,
            owner: owner,
            repo: repo,
            perPage: perPage,
    ...
    func getEnvironment(owner: String, repo: String, environmentName: String) async throws -> Environment {
        try await ReposMethods.reposGetEnvironment(
            config: config,
            owner: owner,
            repo: repo,
    ...
    func createOrUpdateEnvironment(
        owner: String,
        repo: String,
        ...
        try await ReposMethods.reposCreateOrUpdateEnvironment(
            config: config,
            owner: owner,
            repo: repo,
            environmentName: environmentName,
    ...
    func deleteAnEnvironment(owner: String, repo: String, environmentName: String) async throws -> SdkEmptyResponse {
        try await ReposMethods.reposDeleteAnEnvironment(
            config: config,
            owner: owner,
            repo: repo,
            environmentName: environmentName
    ...
    /// Lists the deployment branch policies for an environment. Anyone with read access to the repository can use this
    /// endpoint. OAuth app tokens and personal access tokens (classic) need the `repo` scope to use this endpoint with
    /// a private repository.
    func listDeploymentBranchPolicies(
        owner: String,
        repo: String,
        environmentName: String,
        perPage: Int?,
        page: Int?
    ) async throws -> ReposListDeploymentBranchPoliciesResponse {
        try await ReposMethods.reposListDeploymentBranchPolicies(
            config: config,
            owner: owner,
            repo: repo,
            environmentName: environmentName,
    ...
    /// (classic) need the `repo` scope to use this endpoint.
    func createDeploymentBranchPolicy(
        owner: String,
        repo: String,
        environmentName: String,
        name: String,
        ...
        try await ReposMethods.reposCreateDeploymentBranchPolicy(
            config: config,
            owner: owner,
            repo: repo,
            environmentName: environmentName,
...
public extension ReposNamespace {
    ...
    func getDeploymentBranchPolicy(
        owner: String,
        repo: String,
        environmentName: String,
        branchPolicyId: Int
    ) async throws -> DeploymentBranchPolicy {
        try await ReposMethods.reposGetDeploymentBranchPolicy(
            config: config,
            owner: owner,
            repo: repo,
            environmentName: environmentName,
    ...
    /// (classic) need the `repo` scope to use this endpoint.
    func updateDeploymentBranchPolicy(
        owner: String,
        repo: String,
        environmentName: String,
        branchPolicyId: Int,
        ...
        try await ReposMethods.reposUpdateDeploymentBranchPolicy(
            config: config,
            owner: owner,
            repo: repo,
            environmentName: environmentName,
    ...
    /// (classic) need the `repo` scope to use this endpoint.
    func deleteDeploymentBranchPolicy(
        owner: String,
        repo: String,
        environmentName: String,
        branchPolicyId: Int
    ) async throws -> SdkEmptyResponse {
        try await ReposMethods.reposDeleteDeploymentBranchPolicy(
            config: config,
            owner: owner,
            repo: repo,
            environmentName: environmentName,
    ...
    func getAllDeploymentProtectionRules(
        environmentName: String,
        repo: String,
        owner: String
    ) async throws -> ReposGetAllDeploymentProtectionRulesResponse {
        try await ReposMethods.reposGetAllDeploymentProtectionRules(
            config: config,
            environmentName: environmentName,
            repo: repo,
            owner: owner
    ...
    /// OAuth app tokens and personal access tokens (classic) need the `repo` scope to use this endpoint.
    func createDeploymentProtectionRule(
        environmentName: String,
        repo: String,
        owner: String,
        integrationId: Int?
    ) async throws -> DeploymentProtectionRule {
        try await ReposMethods.reposCreateDeploymentProtectionRule(
            config: config,
            environmentName: environmentName,
            repo: repo,
            owner: owner,
    ...
    func listCustomDeploymentRuleIntegrations(
        environmentName: String,
        repo: String,
        owner: String,
        page: Int?,
        ...
        try await ReposMethods.reposListCustomDeploymentRuleIntegrations(
            config: config,
            environmentName: environmentName,
            repo: repo,
            owner: owner,
    ...
    func getCustomDeploymentProtectionRule(
        owner: String,
        repo: String,
        environmentName: String,
        protectionRuleId: Int
    ) async throws -> DeploymentProtectionRule {
        try await ReposMethods.reposGetCustomDeploymentProtectionRule(
            config: config,
            owner: owner,
            repo: repo,
            environmentName: environmentName,
    ...
    func disableDeploymentProtectionRule(
        environmentName: String,
        repo: String,
        owner: String,
        protectionRuleId: Int
    ) async throws -> SdkEmptyResponse {
        try await ReposMethods.reposDisableDeploymentProtectionRule(
            config: config,
            environmentName: environmentName,
            repo: repo,
            owner: owner,
