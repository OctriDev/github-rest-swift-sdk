...
extension DependencyGraphMethods {
    ...
    public static func dependencyGraphExportSbom(config: ClientConfig, owner: String, repo: String) async throws -> DependencyGraphSpdxSbom {
        return try (await sdkRequest("GET", ["/repos/", sdkEncodePathSegment(sdkWireString(owner)), "/", sdkEncodePathSegment(sdkWireString(repo)), "/dependency-graph/sbom"].joined(), config: config, decoder: .json, operationId: "dependencyGraphExportSbom")).data
    }
    ...
    public static func dependencyGraphFetchSbomReport(config: ClientConfig, owner: String, repo: String, sbomUuid: String) async throws -> SdkEmptyResponse {
        return try (await sdkRequest("GET", ["/repos/", sdkEncodePathSegment(sdkWireString(owner)), "/", sdkEncodePathSegment(sdkWireString(repo)), "/dependency-graph/sbom/fetch-report/", sdkEncodePathSegment(sdkWireString(sbomUuid))].joined(), config: config, decoder: .empty, operationId: "dependencyGraphFetchSbomReport")).data
    }
}
