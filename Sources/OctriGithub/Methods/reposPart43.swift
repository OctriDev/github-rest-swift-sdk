...
extension ReposMethods {
    ...
    public static func reposRemoveTeamAccessRestrictions(config: ClientConfig, owner: String, repo: String, branch: String, body: ReposRemoveTeamAccessRestrictionsRequestBody?) async throws -> [Team] {
        return try (await sdkRequest("DELETE", ["/repos/", sdkEncodePathSegment(sdkWireString(owner)), "/", sdkEncodePathSegment(sdkWireString(repo)), "/branches/", sdkEncodePathSegment(sdkWireString(branch)), "/protection/restrictions/teams"].joined(), config: config, rawBody: (try body.map { try sdkJsonEncoder().encode($0) }), decoder: .json, operationId: "reposRemoveTeamAccessRestrictions")).data
    }
}
