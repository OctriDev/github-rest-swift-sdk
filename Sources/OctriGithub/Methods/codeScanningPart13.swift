...
extension CodeScanningMethods {
    ...
    public static func codeScanningGetAnalysis(config: ClientConfig, owner: String, repo: String, analysisId: Int) async throws -> CodeScanningAnalysis {
        return try (await sdkRequest("GET", ["/repos/", sdkEncodePathSegment(sdkWireString(owner)), "/", sdkEncodePathSegment(sdkWireString(repo)), "/code-scanning/analyses/", sdkEncodePathSegment(sdkWireString(analysisId))].joined(), config: config, decoder: .json, operationId: "codeScanningGetAnalysis")).data
    }
}
