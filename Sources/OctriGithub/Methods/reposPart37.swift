...
extension ReposMethods {
    ...
    public static func reposAddAppAccessRestrictions(config: ClientConfig, owner: String, repo: String, branch: String, apps: [String]) async throws -> [Integration?] {
        let requestBody = ReposAddAppAccessRestrictionsRequestBody(apps: apps)

        return try (await sdkRequest("POST", ["/repos/", sdkEncodePathSegment(sdkWireString(owner)), "/", sdkEncodePathSegment(sdkWireString(repo)), "/branches/", sdkEncodePathSegment(sdkWireString(branch)), "/protection/restrictions/apps"].joined(), config: config, body: requestBody, decoder: .json, operationId: "reposAddAppAccessRestrictions")).data
    }
}
