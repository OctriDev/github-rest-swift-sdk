...
extension PackagesMethods {
    ...
    public static func packagesDeletePackageForUser(config: ClientConfig, packageType: PackagesDeletePackageForOrgParameter, packageName: String, username: String) async throws -> SdkEmptyResponse {
        return try (await sdkRequest("DELETE", ["/users/", sdkEncodePathSegment(sdkWireString(username)), "/packages/", sdkEncodePathSegment(sdkWireString(packageType)), "/", sdkEncodePathSegment(sdkWireString(packageName))].joined(), config: config, decoder: .empty, operationId: "packagesDeletePackageForUser")).data
    }
}
