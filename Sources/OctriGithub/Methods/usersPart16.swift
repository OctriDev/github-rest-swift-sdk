...
public extension UsersMethods {
    ...
    static func usersDeleteSshSigningKeyForAuthenticatedUser(
        config: ClientConfig,
        sshSigningKeyId: Int
    ...
    static func usersGetById(config: ClientConfig, accountId: Int) async throws -> UsersGetByIdResponse {
        try await (sdkRequest(
            "GET",
            ["/user/", sdkEncodePathSegment(sdkWireString(accountId))].joined(),
            config: config,
            decoder: .json,
