...
extension PackagesMethods {
    ...
    /// - token: package token
    public static func packagesRestorePackageForOrg(config: ClientConfig, packageType: PackagesDeletePackageForOrgParameter, packageName: String, org: String, token: String?) async throws -> SdkEmptyResponse {
        return try (await sdkRequest("POST", ["/orgs/", sdkEncodePathSegment(sdkWireString(org)), "/packages/", sdkEncodePathSegment(sdkWireString(packageType)), "/", sdkEncodePathSegment(sdkWireString(packageName)), "/restore"].joined(), config: config, query: [
            SdkQueryParameter("token", value: token),
        ], decoder: .empty, operationId: "packagesRestorePackageForOrg")).data
    }
}
