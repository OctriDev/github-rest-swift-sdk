...
extension PackagesMethods {
    ...
    public static func packagesRestorePackageVersionForUser(config: ClientConfig, packageType: PackagesDeletePackageForOrgParameter, packageName: String, username: String, packageVersionId: Int) async throws -> SdkEmptyResponse {
        return try (await sdkRequest("POST", ["/users/", sdkEncodePathSegment(sdkWireString(username)), "/packages/", sdkEncodePathSegment(sdkWireString(packageType)), "/", sdkEncodePathSegment(sdkWireString(packageName)), "/versions/", sdkEncodePathSegment(sdkWireString(packageVersionId)), "/restore"].joined(), config: config, decoder: .empty, operationId: "packagesRestorePackageVersionForUser")).data
    }
}
