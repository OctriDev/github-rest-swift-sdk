...
extension CopilotMethods {
    ...
    public static func copilotCopilotEnterpriseUserTeamsOneDayReport(config: ClientConfig, enterprise: String, day: String) async throws -> CopilotUsageMetrics1DayReport {
        try sdkValidateDate("day", day)

        return try (await sdkRequest("GET", ["/enterprises/", sdkEncodePathSegment(sdkWireString(enterprise)), "/copilot/metrics/reports/user-teams-1-day"].joined(), config: config, query: [
            SdkQueryParameter("day", value: day),
        ], decoder: .json, operationId: "copilotCopilotEnterpriseUserTeamsOneDayReport")).data
    }
    ...
    /// - enterprise: The slug version of the enterprise name.
    /// - day: The day to request data for, in `YYYY-MM-DD` format.
    public static func copilotCopilotUsersOneDayUsageMetrics(config: ClientConfig, enterprise: String, day: String) async throws -> CopilotUsageMetrics1DayReport {
        try sdkValidateDate("day", day)

        return try (await sdkRequest("GET", ["/enterprises/", sdkEncodePathSegment(sdkWireString(enterprise)), "/copilot/metrics/reports/users-1-day"].joined(), config: config, query: [
            SdkQueryParameter("day", value: day),
        ], decoder: .json, operationId: "copilotCopilotUsersOneDayUsageMetrics")).data
    }
    ...
    /// - Parameters:
    /// - enterprise: The slug version of the enterprise name.
    public static func copilotCopilotUsersUsageMetrics(config: ClientConfig, enterprise: String) async throws -> CopilotUsageMetrics28DayReport {
        return try (await sdkRequest("GET", ["/enterprises/", sdkEncodePathSegment(sdkWireString(enterprise)), "/copilot/metrics/reports/users-28-day/latest"].joined(), config: config, decoder: .json, operationId: "copilotCopilotUsersUsageMetrics")).data
    }
}
