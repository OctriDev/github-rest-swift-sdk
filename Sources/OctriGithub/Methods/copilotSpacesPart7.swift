...
extension CopilotSpacesMethods {
    ...
    public static func copilotSpacesListResourcesForOrg(config: ClientConfig, org: String, spaceNumber: Int) async throws -> CopilotSpacesListResourcesForOrgResponse {
        return try (await sdkRequest("GET", ["/orgs/", sdkEncodePathSegment(sdkWireString(org)), "/copilot-spaces/", sdkEncodePathSegment(sdkWireString(spaceNumber)), "/resources"].joined(), config: config, decoder: .json, operationId: "copilotSpacesListResourcesForOrg")).data
    }
    ...
    public static func copilotSpacesCreateResourceForOrg(config: ClientConfig, org: String, spaceNumber: Int, resourceType: CopilotSpacesCreateResourceForOrgRequestBodyResourceType, metadata: [String: JSONValue]) async throws -> CopilotSpaceResource {
        let requestBody = CopilotSpacesCreateResourceForOrgRequestBody(resourceType: resourceType, metadata: metadata)

        return try (await sdkRequest("POST", ["/orgs/", sdkEncodePathSegment(sdkWireString(org)), "/copilot-spaces/", sdkEncodePathSegment(sdkWireString(spaceNumber)), "/resources"].joined(), config: config, body: requestBody, decoder: .json, operationId: "copilotSpacesCreateResourceForOrg")).data
    }
}
