...
extension TeamsMethods {
    ...
    public static func teamsAddOrUpdateMembershipForUserLegacy(config: ClientConfig, teamId: Int, username: String, role: TeamsAddOrUpdateMembershipForUserLegacyRequestBodyRole?) async throws -> TeamMembership {
        let requestBody = TeamsAddOrUpdateMembershipForUserLegacyRequestBody(role: role)

        return try (await sdkRequest("PUT", ["/teams/", sdkEncodePathSegment(sdkWireString(teamId)), "/memberships/", sdkEncodePathSegment(sdkWireString(username))].joined(), config: config, body: requestBody, decoder: .json, operationId: "teamsAddOrUpdateMembershipForUserLegacy")).data
    }
}
