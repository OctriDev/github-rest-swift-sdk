...
extension OrgsMethods {
    ...
    public static func orgsListAttestationsBulk(config: ClientConfig, org: String, subjectDigests: [String], perPage: Int?, before: String?, after: String?, predicateType: String?) async throws -> OrgsListAttestationsBulkResponse {
        try validateItems("subject_digests", subjectDigests, min: 1, max: 1024)

        let requestBody = OrgsListAttestationsBulkRequestBody(subjectDigests: subjectDigests, predicateType: predicateType)

        return try (await sdkRequest("POST", ["/orgs/", sdkEncodePathSegment(sdkWireString(org)), "/attestations/bulk-list"].joined(), config: config, query: [
            SdkQueryParameter("per_page", value: perPage),
            SdkQueryParameter("before", value: before),
            SdkQueryParameter("after", value: after),
        ], body: requestBody, decoder: .json, operationId: "orgsListAttestationsBulk")).data
    }
}
