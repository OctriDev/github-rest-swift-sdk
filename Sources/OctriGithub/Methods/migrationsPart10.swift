...
extension MigrationsMethods {
    ...
    public static func migrationsMapCommitAuthor(config: ClientConfig, owner: String, repo: String, authorId: Int, email: String?, name: String?) async throws -> PorterAuthor {
        let requestBody = MigrationsMapCommitAuthorRequestBody(email: email, name: name)

        return try (await sdkRequest("PATCH", ["/repos/", sdkEncodePathSegment(sdkWireString(owner)), "/", sdkEncodePathSegment(sdkWireString(repo)), "/import/authors/", sdkEncodePathSegment(sdkWireString(authorId))].joined(), config: config, body: requestBody, decoder: .json, operationId: "migrationsMapCommitAuthor")).data
    }
}
