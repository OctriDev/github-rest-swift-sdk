...
extension EnterpriseTeamsMethods {
    ...
    public static func enterpriseTeamsGet(config: ClientConfig, enterprise: String, teamSlug: String) async throws -> EnterpriseTeamWithMemberCount {
        return try (await sdkRequest("GET", ["/enterprises/", sdkEncodePathSegment(sdkWireString(enterprise)), "/teams/", sdkEncodePathSegment(sdkWireString(teamSlug))].joined(), config: config, decoder: .json, operationId: "enterpriseTeamsGet")).data
    }
}
