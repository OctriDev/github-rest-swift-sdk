...
extension PackagesMethods {
    ...
    public static func packagesDeletePackageVersionForUser(config: ClientConfig, packageType: PackagesDeletePackageForOrgParameter, packageName: String, username: String, packageVersionId: Int) async throws -> SdkEmptyResponse {
        return try (await sdkRequest("DELETE", ["/users/", sdkEncodePathSegment(sdkWireString(username)), "/packages/", sdkEncodePathSegment(sdkWireString(packageType)), "/", sdkEncodePathSegment(sdkWireString(packageName)), "/versions/", sdkEncodePathSegment(sdkWireString(packageVersionId))].joined(), config: config, decoder: .empty, operationId: "packagesDeletePackageVersionForUser")).data
    }
}
