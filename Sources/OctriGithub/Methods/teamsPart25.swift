...
extension TeamsMethods {
    ...
    public static func teamsRemoveRepoLegacy(config: ClientConfig, teamId: Int, owner: String, repo: String) async throws -> SdkEmptyResponse {
        return try (await sdkRequest("DELETE", ["/teams/", sdkEncodePathSegment(sdkWireString(teamId)), "/repos/", sdkEncodePathSegment(sdkWireString(owner)), "/", sdkEncodePathSegment(sdkWireString(repo))].joined(), config: config, decoder: .empty, operationId: "teamsRemoveRepoLegacy")).data
    }
}
