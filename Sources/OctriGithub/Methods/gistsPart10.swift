...
public extension GistsMethods {
    ...
    static func gistsStar(config: ClientConfig, gistId: String) async throws -> SdkEmptyResponse {
    ...
    /// - Parameters:
    /// - gistId: The unique identifier of the gist.
    static func gistsUnstar(config: ClientConfig, gistId: String) async throws -> SdkEmptyResponse {
        try await (sdkRequest(
            "DELETE",
            ["/gists/", sdkEncodePathSegment(sdkWireString(gistId)), "/star"].joined(),
            config: config,
    ...
    /// - Parameters:
    /// - gistId: The unique identifier of the gist.
    static func gistsGetRevision(config: ClientConfig, gistId: String, sha: String) async throws -> GistSimple {
        try await (sdkRequest(
            "GET",
            ["/gists/", sdkEncodePathSegment(sdkWireString(gistId)), "/", sdkEncodePathSegment(sdkWireString(sha))]
                .joined(),
            config: config,
            decoder: .json,
