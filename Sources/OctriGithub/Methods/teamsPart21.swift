...
extension TeamsMethods {
    ...
    public static func teamsRemoveMembershipForUserLegacy(config: ClientConfig, teamId: Int, username: String) async throws -> SdkEmptyResponse {
        return try (await sdkRequest("DELETE", ["/teams/", sdkEncodePathSegment(sdkWireString(teamId)), "/memberships/", sdkEncodePathSegment(sdkWireString(username))].joined(), config: config, decoder: .empty, operationId: "teamsRemoveMembershipForUserLegacy")).data
    }
}
