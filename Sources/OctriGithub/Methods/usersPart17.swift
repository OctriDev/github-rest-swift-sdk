...
extension UsersMethods {
    ...
    ///   -rest-api)."
    public static func usersList(config: ClientConfig, since: Int?, perPage: Int?) async throws -> [SimpleUser] {
        return try (await sdkRequest("GET", "/users", config: config, query: [
    ...
    public static func usersGetByUsername(config: ClientConfig, username: String) async throws -> UsersGetByUsernameResponse {
        return try (await sdkRequest("GET", ["/users/", sdkEncodePathSegment(sdkWireString(username))].joined(), config: config, decoder: .json, operationId: "usersGetByUsername")).data
    }
}
