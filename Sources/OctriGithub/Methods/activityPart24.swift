...
extension ActivityMethods {
    ...
    ///   API](https://docs.github.com/rest/using-the-rest-api/using-pagination-in-the
    ///   -rest-api)."
    public static func activityListReceivedEventsForUser(config: ClientConfig, username: String, perPage: Int?, page: Int?) async throws -> [Event] {
        return try (await sdkRequest("GET", ["/users/", sdkEncodePathSegment(sdkWireString(username)), "/received_events"].joined(), config: config, query: [
            SdkQueryParameter("per_page", value: perPage),
            SdkQueryParameter("page", value: page),
        ], decoder: .json, operationId: "activityListReceivedEventsForUser")).data
    }
}
