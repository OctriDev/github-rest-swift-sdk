...
extension ReposMethods {
    ...
    public static func reposRenameBranch(config: ClientConfig, owner: String, repo: String, branch: String, newName: String) async throws -> BranchWithProtection {
        let requestBody = ReposRenameBranchRequestBody(newName: newName)

        return try (await sdkRequest("POST", ["/repos/", sdkEncodePathSegment(sdkWireString(owner)), "/", sdkEncodePathSegment(sdkWireString(repo)), "/branches/", sdkEncodePathSegment(sdkWireString(branch)), "/rename"].joined(), config: config, body: requestBody, decoder: .json, operationId: "reposRenameBranch")).data
    }
}
