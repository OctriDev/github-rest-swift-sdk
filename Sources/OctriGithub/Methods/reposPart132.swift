...
extension ReposMethods {
    ...
    public static func reposUploadReleaseAsset(config: ClientConfig, owner: String, repo: String, releaseId: Int, name: String, label: String?, body: Data?) async throws -> ReleaseAsset {
        return try (await sdkRequest("POST", ["/repos/", sdkEncodePathSegment(sdkWireString(owner)), "/", sdkEncodePathSegment(sdkWireString(repo)), "/releases/", sdkEncodePathSegment(sdkWireString(releaseId)), "/assets"].joined(), config: config, query: [
            SdkQueryParameter("name", value: name),
            SdkQueryParameter("label", value: label),
        ], rawBody: body, contentType: "application/octet-stream", decoder: .json, operationId: "reposUploadReleaseAsset")).data
    }
}
