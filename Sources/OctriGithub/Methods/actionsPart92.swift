...
extension ActionsMethods {
    ...
    ///   API](https://docs.github.com/rest/using-the-rest-api/using-pagination-in-the
    ///   -rest-api)."
    public static func actionsListRepoSecrets(config: ClientConfig, owner: String, repo: String, perPage: Int?, page: Int?) async throws -> ActionsListRepoSecretsResponse {
        return try (await sdkRequest("GET", ["/repos/", sdkEncodePathSegment(sdkWireString(owner)), "/", sdkEncodePathSegment(sdkWireString(repo)), "/actions/secrets"].joined(), config: config, query: [
            SdkQueryParameter("per_page", value: perPage),
            SdkQueryParameter("page", value: page),
        ], decoder: .json, operationId: "actionsListRepoSecrets")).data
    }
}
