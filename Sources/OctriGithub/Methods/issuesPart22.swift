...
extension IssuesMethods {
    ...
    public static func issuesRemoveDependencyBlockedBy(config: ClientConfig, owner: String, repo: String, issueNumber: Int, issueId: Int) async throws -> Issue {
        return try (await sdkRequest("DELETE", ["/repos/", sdkEncodePathSegment(sdkWireString(owner)), "/", sdkEncodePathSegment(sdkWireString(repo)), "/issues/", sdkEncodePathSegment(sdkWireString(issueNumber)), "/dependencies/blocked_by/", sdkEncodePathSegment(sdkWireString(issueId))].joined(), config: config, decoder: .json, operationId: "issuesRemoveDependencyBlockedBy")).data
    }
}
