...
extension GitMethods {
    ...
    ///   not case sensitive.
    public static func gitGetTag(config: ClientConfig, owner: String, repo: String, tagSha: String) async throws -> GitTag {
        return try (await sdkRequest("GET", ["/repos/", sdkEncodePathSegment(sdkWireString(owner)), "/", sdkEncodePathSegment(sdkWireString(repo)), "/git/tags/", sdkEncodePathSegment(sdkWireString(tagSha))].joined(), config: config, decoder: .json, operationId: "gitGetTag")).data
    }
}
