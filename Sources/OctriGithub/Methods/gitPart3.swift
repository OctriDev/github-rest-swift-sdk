...
extension GitMethods {
    public struct GitCreateCommitOptions: Codable {
    ...
    ///   signed commits.
    public static func gitCreateCommit(config: ClientConfig, options: GitCreateCommitOptions) async throws -> GitCommit {
        let requestBody = GitCreateCommitRequestBody(options: options)

        return try (await sdkRequest("POST", ["/repos/", sdkEncodePathSegment(sdkWireString(options.owner)), "/", sdkEncodePathSegment(sdkWireString(options.repo)), "/git/commits"].joined(), config: config, body: requestBody, decoder: .json, operationId: "gitCreateCommit")).data
    }
}
