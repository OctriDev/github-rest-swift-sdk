...
public class SearchNamespace {
    ...
    /// page](https://docs.github.com/rest/guides/using-pagination-in-the-rest-api). When searching for code, you can
    /// get text match metadata for the file **content** and file **path** fields when you pass the `text-match` media
    ...
    /// definition of the `addClass` function inside [jQuery](https://github.com/jquery/jquery) repository, your query
    /// would look something like this: `q=addClass+in:file+language:js+repo:jquery/jquery` This query searches for the
    /// keyword `addClass` within a file's contents. The query limits the search to files where the language is
    /// JavaScript in the `jquery/jquery` repository. Considerations for code search: Due to the complexity of searching
    /// code, there are a few restrictions on how searches are performed: * Only the _default branch_ is considered. In
    /// most cases, this will be the `master` branch. * Only files smaller than 384 KB are searchable. * You must always
    /// include at least one search term when searching source code. For example, searching for
    /// [`language:go`](https://github.com/search?utf8=%E2%9C%93&q=language%3Ago&type=Code) is not valid, while
    /// [`amazing language:go`](https://github.com/search?utf8=%E2%9C%93&q=amazing+language%3Ago&type=Code) is. This
    /// endpoint requires you to authenticate and limits you to 10 requests per minute.
    public func code(
        q: String,
        sort: SearchCodeParameter?,
        order: SearchCodeParameterX5b868389?,
        perPage: Int?,
        page: Int?
    ) async throws -> SearchCodeResponse {
        try await SearchMethods.searchCode(config: config, q: q, sort: sort, order: order, perPage: perPage, page: page)
    }
    ...
    public func commits(
        ...
        try await SearchMethods.searchCommits(
            config: config,
            q: q,
            sort: sort,
            order: order,
    ...
    public func issuesAndPullRequests(
        q: String,
        ...
        try await SearchMethods.searchIssuesAndPullRequests(
            config: config,
            q: q,
            sort: sort,
            order: order,
    ...
    public func labels(
        ...
        try await SearchMethods.searchLabels(
            config: config,
    ...
    public func repos(
        ...
        try await SearchMethods.searchRepos(
            config: config,
            q: q,
            sort: sort,
            order: order,
    ...
    public func topics(q: String, perPage: Int?, page: Int?) async throws -> SearchTopicsResponse {
        try await SearchMethods.searchTopics(config: config, q: q, perPage: perPage, page: page)
    }
    ...
    public func users(
        ...
        try await SearchMethods.searchUsers(
            config: config,
            q: q,
            sort: sort,
            order: order,
...
public class UsersNamespace {
    ...
    public func getAuthenticated() async throws -> UsersGetAuthenticatedResponse {
        try await UsersMethods.usersGetAuthenticated(config: config)
    }
    ...
    public func updateAuthenticated(options: UsersMethods.UsersUpdateAuthenticatedOptions) async throws -> PrivateUser {
        try await UsersMethods.usersUpdateAuthenticated(config: config, options: options)
    }
    ...
    public func listBlockedByAuthenticatedUser(perPage: Int?, page: Int?) async throws -> [SimpleUser] {
        try await UsersMethods.usersListBlockedByAuthenticatedUser(config: config, perPage: perPage, page: page)
    }
    ...
    public func checkBlocked(username: String) async throws -> SdkEmptyResponse {
        try await UsersMethods.usersCheckBlocked(config: config, username: username)
    }
    ...
    public func block(username: String) async throws -> SdkEmptyResponse {
        try await UsersMethods.usersBlock(config: config, username: username)
    }
    ...
    public func unblock(username: String) async throws -> SdkEmptyResponse {
        try await UsersMethods.usersUnblock(config: config, username: username)
    }

    /// Set primary email visibility for the authenticated user
    ///
    /// Sets the visibility for your primary email addresses.
    public func setPrimaryEmailVisibilityForAuthenticatedUser(
        visibility: UsersSetPrimaryEmailVisibilityForAuthenticatedUserRequestBodyVisibility
    ) async throws
        -> [Email] {
        try await UsersMethods.usersSetPrimaryEmailVisibilityForAuthenticatedUser(
            config: config,
    ...
    public func listEmailsForAuthenticatedUser(perPage: Int?, page: Int?) async throws -> [Email] {
        try await UsersMethods.usersListEmailsForAuthenticatedUser(config: config, perPage: perPage, page: page)
    }
}
...
public extension UsersNamespace {
    ...
    func addEmailForAuthenticatedUser(body: UsersAddEmailForAuthenticatedUserRequestBody?) async throws -> [Email] {
        try await UsersMethods.usersAddEmailForAuthenticatedUser(config: config, body: body)
    }

    /// Delete an email address for the authenticated user
    ///
    /// OAuth app tokens and personal access tokens (classic) need the `user` scope to use this endpoint.
    func deleteEmailForAuthenticatedUser(body: UsersDeleteEmailForAuthenticatedUserRequestBody?) async throws
        -> SdkEmptyResponse {
        try await UsersMethods.usersDeleteEmailForAuthenticatedUser(config: config, body: body)
    }

    /// List followers of the authenticated user
    ///
    /// Lists the people following the authenticated user.
    func listFollowersForAuthenticatedUser(perPage: Int?, page: Int?) async throws -> [SimpleUser] {
        try await UsersMethods.usersListFollowersForAuthenticatedUser(config: config, perPage: perPage, page: page)
    }

    /// List the people the authenticated user follows
    ///
    /// Lists the people who the authenticated user follows.
    func listFollowedByAuthenticatedUser(perPage: Int?, page: Int?) async throws -> [SimpleUser] {
        try await UsersMethods.usersListFollowedByAuthenticatedUser(config: config, perPage: perPage, page: page)
    }

    /// Check if a person is followed by the authenticated user
    func checkPersonIsFollowedByAuthenticated(username: String) async throws -> SdkEmptyResponse {
        try await UsersMethods.usersCheckPersonIsFollowedByAuthenticated(config: config, username: username)
    }
    ...
    func follow(username: String) async throws -> SdkEmptyResponse {
        try await UsersMethods.usersFollow(config: config, username: username)
    }

    /// Unfollow a user
    ///
    /// OAuth app tokens and personal access tokens (classic) need the `user:follow` scope to use this endpoint.
    func unfollow(username: String) async throws -> SdkEmptyResponse {
        try await UsersMethods.usersUnfollow(config: config, username: username)
    }
    ...
    func listGpgKeysForAuthenticatedUser(perPage: Int?, page: Int?) async throws -> [GpgKey] {
        try await UsersMethods.usersListGpgKeysForAuthenticatedUser(config: config, perPage: perPage, page: page)
    }
}
